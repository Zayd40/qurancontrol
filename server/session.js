function createSessionManager({ metadata, quranDataset, duasById, eventsById }) {
  ...
  function getQuranContentPayload(state) {
    ...
    return {
      ...
      instruction: '',
      repeat: '',
      reference: `Qur'an ${ayah.surahNumber}:${ayah.ayahNumber}`,
      arabic: ayah.arabic,
      transliteration: ayah.transliteration,
  ...
  function getDuaContentPayload(state) {
    ...
    return {
      ...
      instruction: '',
      repeat: '',
      reference: `Line ${lineIndex} of ${dua.lines.length}`,
      arabic: line.arabic || '—',
  ...
  function getGuidedEventContentPayload(state) {
    ...
    return {
      ...
      reference:
        slide.reference ||
        `Section ${guidedEvent.sectionIndex + 1} of ${event.sections.length} · Slide ${guidedEvent.slideIndex + 1} of ${section.slides.length}`,
      arabic: slide.arabic || '',
